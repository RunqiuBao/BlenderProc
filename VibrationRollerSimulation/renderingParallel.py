...
if __name__ == "__main__":
    ...
    for indexToIndices, camInWorldTransform in enumerate(poses):
        ...
        cv2.imwrite('output/seq/leftCam/{}.png'.format(str(poseIndices[indexToIndices]).zfill(6)), cv2.cvtColor(leftImage, cv2.COLOR_RGB2BGR))
        cv2.imwrite('output/seq/rightCam/{}.png'.format(str(poseIndices[indexToIndices]).zfill(6)), cv2.cvtColor(rightImage, cv2.COLOR_RGB2BGR))
